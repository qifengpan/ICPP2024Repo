...
subroutine computeNext(x0, x, lda1, lda2, lda3, dt, hx, hy, hz, r, me, &
                       xs, ys, zs, xe, ye, ze, nproc, k0, nSample)
   !use cudafor
   ! Input parameters
   integer lda1, lda2, lda3, me, nproc, nSample
   integer xs(0:nproc-1), ys(0:nproc-1), xe(0:nproc-1), ye(0:nproc-1), ze(0:nproc-1), &
           zs(0:nproc-1)
   double precision x(0:nSample-1,0:lda1-1,0:lda2-1,0:lda3-1)
!   double precision,device :: x_d(0:nSample-1,0:lda1-1,0:lda2-1,0:lda3-1),&
!                              x0_d(0:nSample-1,0:lda1-1,0:lda2-1,0:lda3-1)
   double precision dt, hx, hy, hz
   double precision,dimension(0:nSample-1) :: k0

   ! Output array
   double precision x0(0:nSample-1,0:lda1-1,0:lda2-1,0:lda3-1)

   ! Output error
   double precision,dimension(0:nSample-1):: r,rk
...
!    double precision rk
   ...
   double precision,dimension(0:nSample-1):: diagx, diagy, diagz, weightx, weighty, weightz
!   double precision,dimension(0:nSample-1), device :: diagx_d,diagy_d,diagz_d,weigthx_d,&
!                                                     weighty_d,weightz_d
...
!   grid   = dim3(ceiling(real(lda1)/tBlock%x),&
!                 ceiling(real(lda2)/tBlock%y),&
!                 ceiling(real(lda3)/tBlock%z)) 
!   x0_d   = x0
!   weightx_d = weightx
!   weighty_d = weighty
!   weightz_d = weightz
...
!   diagy_d   = diagy
!   diagz_d   = diag
!   call loop_kernel<<<grid,tBlock>>>(x_d,x0_d,weightx_d,weighty_d,weightz_d,diagx_d,diagy_d,diagz_d)
!   x         = x_d
...
subroutine initValues(nb_layers, x0, size_tot_x, size_tot_y, size_tot_z, nSample, temp1_init, temp2_init)

   ! Input parameters
   integer size_tot_x, size_tot_y, size_tot_z, nSample
   ! Init Temperatures
   double precision temp1_init, temp2_init

   ! Output array
   double precision x0(0:nSample-1,0:size_tot_x-1,0:size_tot_y-1,0:size_tot_z-1)
...
!attributes(global) subroutine loop_kernel(x_d,x0_d,weightx_d,weighty_d,weightz_d,diagx_d,diagy_d,diagz_d)
!implicit none
!double precision, dimension(:,:,:,:) :: x_d,x0_d
!double precision, dimension(:)       :: weightx_d,weighty_d,weightz_d,diagx_d,diagy_d,diagz_d
!!local variables 
!integer :: i,j,k
!
...
!
!x_d(:,i,j,k) = weightx_d*(x0_d(:,i-1,j,k)  + x0_d(:,i+1,j,k) +  x0_d(:,i,j,k)*diagx_d) + &
!               weighty_d*(x0_d(:,i,j-1,k)  + x0_d(:,i,j+1,k) +  x0_d(:,i,j,k)*diagy_d) + &
!               weightz_d*(x0_d(:,i,j,k-1)  + x0_d(:,i,j,k+1) +  x0_d(:,i,j,k)*diagz_d) 
!end subroutine loop_kernel
