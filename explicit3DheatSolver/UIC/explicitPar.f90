...
program heat
   ...
   double precision, dimension(50) :: resLoc, result,k0
   ...
   ! MPI variables
   integer, dimension(4) :: sizes, subsizes1, subsizes2, subsizes3, starts
...
!   k0 = (/0.95,0.96,0.97,0.98,0.99,1.0,1.01,1.02,1.03,1.04,1.05,1.06,1.07,1.08,1.09/)
   nSample = 50
!   call generateGauss(k0,nSample)
   k0 = 1
   ! MPI initialization
   ...
   allocate(xfinal(0:nSample-1,size_x*size_y*size_z))
   allocate(x(0:nSample-1,0:size_tot_x-1,0:size_tot_y-1,0:size_tot_z-1))
   allocate(x0(0:nSample-1,0:size_tot_x-1,0:size_tot_y-1,0:size_tot_z-1))
   ...
   ! Allocate subdomain
   allocate(xtemp(0:nSample-1,1:xcell*ycell*zcell))
   ...
   call MPI_Type_create_subarray(4, sizes, subsizes1, starts, &
   ...
   call MPI_Type_create_subarray(4, sizes, subsizes2, starts, &
                                 MPI_ORDER_FORTRAN, MPI_DOUBLE_PRECISION, &
   ...
   call MPI_Type_create_subarray(4, sizes, subsizes3, starts, &
                                 MPI_ORDER_FORTRAN, MPI_DOUBLE_PRECISION, &
                                 matrix_type_oxy, infompi)
   call MPI_Type_commit(matrix_type_oxy, infompi)

   ! Initialize values
   call initValues(nb_layers, x0, size_tot_x, size_tot_y, size_tot_z, nSample, temp1_init, temp2_init)
   ...
   call updateBound(x0, size_tot_x, size_tot_y, size_tot_z, NeighBor, comm3d, &
                    ...
                    xs, ys, zs, xe, ye, ze, nproc, nSample)
...
!    print *,'now is in the ',step,'th step'
   ...
   call MPI_Allreduce(resLoc, result, nSample, MPI_DOUBLE_PRECISION, MPI_SUM, comm, infompi)
   ! Current error
   result = sqrt(result)
...
301 continue
   ...
   ! Perform gathering
   call MPI_Gather(xtemp, nSample*xcell*ycell*zcell, MPI_DOUBLE_PRECISION, xfinal, nSample*xcell*ycell*zcell, &
